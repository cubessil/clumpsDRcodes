...
Standards <- readRDS(url("https://github.com/cubessil/clumpsDRcodes/blob/master/Standards_070319.RDS?raw=true"))
```
...
#Adds acid digestion factor to CDES values - multiple options: 0.092 is from Henkes et al., 2013. 0.082 is from Defliese et al., 2015.
...
CDES.std.pri <-  left_join(CDES.std.pri, select(Standards, Sample.ID, D47.CDES.90.acc, D47.CDES.25H.acc, D47.CDES.25D.acc), by = "Sample.ID")
...
CDES.stds.pri <- CDES.std.pri %>%
  gather(CDES_type, CDES_values, CDES.noAD.pri, CDES.ADH.pri, CDES.ADD.pri) %>%
  gather(CDES_acc_type, CDES_acc_values, D47.CDES.90.acc, D47.CDES.25H.acc, D47.CDES.25D.acc) %>%
  filter(CDES_type == "CDES.noAD.pri" & CDES_acc_type == "D47.CDES.90.acc" | CDES_type == "CDES.ADH.pri" & CDES_acc_type == "D47.CDES.25H.acc" | CDES_type == "CDES.ADD.pri" & CDES_acc_type == "D47.CDES.25D.acc")
...
residual.comparison <- multiplot(CDES.residuals.pri.90, CDES.residuals.sec.bp.90, CDES.residuals.pri.D, CDES.residuals.sec.bp.D, CDES.residuals.pri.H, CDES.residuals.sec.bp.H, cols=3)
...
data.avg <- data %>%
  group_by(Sample.ID) %>%
  filter(D48excess.label == "no D48 excess" & Project == "all") %>%
  summarise(N = n(),
            d13C.se = sd(d13C)/sqrt(N),
            d13C.avg = mean(d13C),
            d18O.se = sd(d18O.VPDB.min)/sqrt(N),
            d18O.avg = mean(d18O.VPDB.min),
            D47.se = sd(CDES.D47stdcorr90.pri)/sqrt(N),
            D47.2se = 2*D47.se,
            D47.90.noAD.avg = mean(CDES.noAD.pri),
            D47.90.avg = mean(CDES.D47stdcorr90.pri),
            D47.25D.avg = mean(CDES.D47stdcorrD.pri),
            D48.se = sd(CDES.48noAD.pri)/sqrt(N),
            D48.2se = 2*D48.se,
            D48.90.avg = mean(CDES.48noAD.pri),
            D48.se.all = sd(CDES.48noAD.pri.all)/sqrt(N),
            D48.2se.all = 2*D48.se.all,
            D48.90.avg.all = mean(CDES.48noAD.pri.all),
...
# unique(data$Project)
# data.nozero <- data %>%
#   filter(Sample.ID != "zero")
# 
# RS <- data.nozero %>%
#   filter(Project == "all" | Project =="RS")
# 
# Thomas <- data.nozero %>%
#   filter(Project == "all" | Project =="Thomas")
# 
# Layzell <- data.nozero %>%
#   filter(Project == "all" | Project =="Layzell")
# 
...
# wb <- add_ws_with_data(wb, "RS", RS)
# wb <- add_ws_with_data(wb, "Thomas", Thomas)
# wb <- add_ws_with_data(wb, "Layzell", Thomas)
